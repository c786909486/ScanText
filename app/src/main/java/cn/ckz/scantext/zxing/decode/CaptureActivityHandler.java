...
import cn.ckz.scantext.zxing.view.ViewfinderResultPointCallback;
...
public final class CaptureActivityHandler extends Handler {
    ...
    public CaptureActivityHandler(CaptureActivity activity, Vector<BarcodeFormat> decodeFormats, String characterSet,String scan_type) {
        this.activity = activity;
        this.scan_type = scan_type;
        decodeThread = new DecodeThread(activity, decodeFormats, characterSet, new ViewfinderResultPointCallback(
                activity.getViewfinderView()),scan_type);
        decodeThread.start();
        state = State.SUCCESS;
    ...
    public void handleMessage(Message message) {
        if (message.what == R.id.auto_focus) {
            ...
            if (scan_type.equals(CaptureActivity.SCAN_TYPE_QRCODESCAN)) {
                activity.handleDecode((Result) message.obj, barcode); //返回二维码解析图片的信息
            } else {
                activity.handleDecodeTwo((String) message.obj, barcode); //返回银行卡和身份证解析图片的信息
            }
            restartPreviewAndDecode();
