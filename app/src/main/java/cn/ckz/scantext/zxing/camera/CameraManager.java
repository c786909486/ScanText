...
package cn.ckz.scantext.zxing.camera;
...
import cn.ckz.scantext.zxing.app.PreferencesActivity;
import cn.ckz.scantext.zxing.decode.PlanarYUVLuminanceSource;
...
public final class CameraManager {
	 ...
	 * 定义扫描框宽高
	 */
	private int MIN_FRAME_WIDTH = 240*2;
	private int MIN_FRAME_HEIGHT = 240*2;
	private int MAX_FRAME_WIDTH = 480*2;
	private int MAX_FRAME_HEIGHT = 360*2;

	public void setFrame(int minWidth,int minHeight,int maxWidth,int maxHeight){
		this.MIN_FRAME_WIDTH = minWidth;
		this.MIN_FRAME_HEIGHT = minHeight;
		this.MAX_FRAME_WIDTH = maxWidth;
		this.MAX_FRAME_HEIGHT = maxHeight;
	}
	 ...
	 */
	public Rect getFramingRect(String type) {
		...
		if (framingRect == null) {
			...
			int width  = 0;
			int height = 0;
			if (type.equals(CaptureActivity.SCAN_TYPE_QRCODESCAN)) {
				...
				width = screenResolution.x * 1;
				height = screenResolution.y * 1 / 3;
				int leftOffset = (screenResolution.x - width) / 2;
...
//			}
//			float densy = context.getResources().getDisplayMetrics().density;
//			int leftOffset = (screenResolution.x - width)/2;
//			int topOffset = (screenResolution.y - height)/2;
//			framingRect = new Rect(leftOffset, topOffset, leftOffset + width,
//					topOffset + height);
//			Log.d(TAG, "Calculated framing rect: " + framingRect);
	 ...
	 * frame, not UI / screen.
	 */
	public Rect getFramingRectInPreview(String type) {
		if (framingRectInPreview == null) {
			Rect rect = new Rect(getFramingRect(type));
	...
	public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data,
		...
		Rect rect = getFramingRectInPreview(type);
